...
| T-057 | Fix Daily Text refresh so browser refresh shows the current day | worker_exec | [x] | Added current-day validation for Daily Text cache/fallback entries, added live-source retry path, refreshed EN/ES snapshots for March 10, and updated the Pi web root cache-bust to `main.js?v=20260310a`. |
| T-058 | Fix stale JW Daily Text display and remove WBC panel from Command Center | worker_exec | [x] | Enforced current-day validation for Daily Text live/fallback/cache paths, removed the WBC card and runtime fetch/render code, and bumped dashboard JS cache-bust to `main.js?v=20260318a`. |
| T-059 | Fix JW Daily Text parser for current WOL markdown format | worker_exec | [x] | Updated runtime/script parsers to support `##` WOL date headings, regenerated EN/ES fallback snapshots for March 18, 2026, and bumped dashboard JS cache-bust to `main.js?v=20260318b`. |
| T-060 | Redeploy Command Center and trace stale Pi-served copy | worker_exec | [x] | Deployed updated dashboard to Vercel alias `https://dashboard-five-theta-88.vercel.app` and verified March 18 Daily Text + no WBC card. Pi-hosted URL `http://10.0.0.115/deliverables/dashboard/` still serves older `main.js?v=20260310a`; remote Pi web-root/timer inspection blocked by SSH auth failure from this session. |
| T-061 | Add top scrolling ticker for weather, dust, and Astros snapshot | worker_exec | [x] | Added right-to-left ticker above the dashboard hero using live weather, Dust & Dander, Astros previous-game score, and next-game timing data; bumped dashboard JS cache-bust to `main.js?v=20260318c`. |
| T-062 | Split live widgets onto separate page and make ticker route there | worker_exec | [x] | Moved weather/allergy/Astros to new `widgets.html`, made the home-page ticker open that page, added a `Live Widgets` button under Operations Pages, and bumped dashboard JS cache-bust to `main.js?v=20260318d`. |
| T-063 | Redesign Live Widgets page and create featured Astros panel | worker_exec | [x] | Reworked `widgets.html` into a fuller destination page, added page-specific layout styling, and upgraded Astros into a featured hero board with larger previous/next game cards; bumped dashboard JS cache-bust to `main.js?v=20260318e`. |
